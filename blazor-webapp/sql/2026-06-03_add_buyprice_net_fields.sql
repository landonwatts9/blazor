...
-- Rebuild gold so column positions match silver (the refresh proc does
-- positional INSERT * SELECT *, so an ALTER TABLE ADD would mismatch on
-- the next refresh).
...
DROP TABLE dbo.EncompassLoan_Gold;
GO

SELECT *
INTO dbo.EncompassLoan_Gold
FROM dbo.vw_EncompassLoan_Silver;
GO

-- SELECT INTO infers NVARCHAR(MAX) for unbounded silver expressions; constrain
-- the folder column so it can serve as a leading key in the pipeline index.
ALTER TABLE dbo.EncompassLoan_Gold ALTER COLUMN currentloanfolder NVARCHAR(100) NULL;
GO
...
CREATE NONCLUSTERED INDEX IX_Gold_funding_date
    ON dbo.EncompassLoan_Gold
    (loanamount_fundedfinal, loanchannel_summary, loanofficer_name, funded_flag, loan_purpose, funding_date);
GO

CREATE NONCLUSTERED INDEX IX_Gold_estclose_pipeline
    ON dbo.EncompassLoan_Gold
    (currentloanfolder, funded_flag, loanchannel_summary, milestone_status, loanofficer_name, loanamount, loanamount_fundedfinal, loan_purpose, estimatedclosing_date);
GO

EXEC dbo.usp_Refresh_EncompassLoan_Gold;
