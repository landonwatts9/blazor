...
BaseColumns AS (
    ...
    CASE
        WHEN [MS.STATUSDATE] IS NULL OR LTRIM(RTRIM(CAST([MS.STATUSDATE] AS NVARCHAR(MAX)))) IN ('', '//') THEN NULL
        WHEN TRY_CONVERT(DATETIME2(0), [MS.STATUSDATE]) < '1582-10-15' THEN NULL
        ELSE TRY_CONVERT(DATETIME2(0), [MS.STATUSDATE])
...
-- Widen the gold column to match silver's new type. ALTER COLUMN works
-- as a direct widening conversion (DATE -> DATETIME2 is lossless), so
-- indexes and column positions stay intact — no rebuild required.
ALTER TABLE dbo.EncompassLoan_Gold
    ALTER COLUMN currentmilestone_date DATETIME2(0) NULL;
GO
